...
!function (jQuery) {
    ...
    ClassToggle.prototype = {
        ...
        classToggle: function (option) {
            ...
            sender = typeof(event) != 'undefined' ? (event ? event.target : window.event.srcElement) : this.options.sender;
            $sender = $(sender);
            
            var tcClass = this.options.classtoggleClass; // $sender.attr('data-classtoggle-class');
            var tcClassAlt = this.options.classtoggleAltclass; //$sender.attr('data-classtoggle-altclass');
            var tcTriggerClass = this.options.classtoggleTriggerActiveclass; // $sender.attr('data-classtoggle-trigger-activeclass');
            var tcTriggerSelector = this.options.classtoggleTriggerSelector; // $sender.attr('data-classtoggle-trigger-selector');
            var target = this.options.classtoggleTarget; // $sender.attr('data-classtoggle-target');
